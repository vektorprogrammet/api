...
import { z } from "zod";
import { futureDateParser, timeStringParser, toDateParser } from "@/lib/time-parsers";
...
export const newInterviewToInsertSchema = newInterviewSchema.extend({
	plannedTime: newInterviewSchema.shape.plannedTime
		.pipe(toDateParser)
		.pipe(futureDateParser),
}).pipe(createInsertSchema(interviewsTable));

export const newInterviewSchemaToInsertSchema = newInterviewSchemaSchema.extend({}) // because of the way drizzle-zod works this pipe does wrong type inference and shouldn't be used


export type NewInterview= z.infer<typeof newInterviewToInsertSchema>;
export type NewInterviewSchema= z.infer<typeof newInterviewSchemaToInsertSchema>;
